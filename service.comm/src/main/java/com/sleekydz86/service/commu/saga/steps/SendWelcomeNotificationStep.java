    ...
    public CompletableFuture<SagaStepResult> execute(Saga saga) {
        return CompletableFuture.supplyAsync(() -> {
            try {
                ...
                request.setUserId(data.getUserId() != null ? Long.parseLong(data.getUserId()) : null);
                request.setMessage("Welcome " + data.getPatientName() + "! Thank you for registering.");
                ...
                return new SagaStepResult(true, "Welcome notification sent successfully", null);
            } catch (Exception e) {
    ...
    public CompletableFuture<SagaStepResult> compensate(Saga saga) {
        return CompletableFuture.supplyAsync(() -> {
            try {
                ...
                return new SagaStepResult(false, "Failed to log notification compensation: " + e.getMessage(), null);
            }
        });
    }
