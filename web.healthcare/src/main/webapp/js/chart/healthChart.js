...
/* !! 대시보드 chart draw start !!  */
/** 
 * barChart draw 함수 (대시보드)
 * @param json {data[],lv[]}
 * @param string (step)
*/
function drawBarChartList(data,chartId){
    var ctx = document.getElementById(chartId);
    
    var config = setBarConfig(data,chartId);

    if(chart[chartId] == null){
        chart[chartId] = new Chart(ctx,config);
    }else{
        chart[chartId].data.labels = data.lv;
        chart[chartId].data.datasets[0].data = data.data;
        chart[chartId].update();
    }
}
/** 
 * Floating Bar Chart draw 함수 (대시보드)
 * @param json {dataMin[], dataMax[], lv[]} 
 * @param string chartId (temperature/spo2/heartrate/stress/bloodpress/repiratory)
*/
function drawFloatingChartList(data,chartId){
    var ctx = document.getElementById(chartId);
    var config = setFloatingConfig(data,chartId);
...
//샘플 차트 draw (list)
function drawStackedChartList(chartId){
    var ctx = document.getElementById(chartId);
            ...
            scales: {
                x: {
                    ...
                    grid: {
                        display: false,
                        drawTicks: false
                        /* color: function (context) {  //label 있을때만 선 표시
                            if (context.tick != null && context.tick != 'undefined') {
