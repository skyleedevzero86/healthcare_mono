...
<%
    ...
    String userRoleFk = (sessionObj != null) ? (String) sessionObj.getAttribute("userRoleFk") : null;
    String acToken = (sessionObj != null) ? (String) sessionObj.getAttribute("acToken") : null;
    ...
    if (userId == null || userId.isEmpty() || acToken == null || acToken.isEmpty()) {
        response.sendRedirect(contextPath + "/user/signin");
        return;
    }
    
    String userinfoId = (sessionObj != null) ? (String) sessionObj.getAttribute("UserinfoId") : null;
...
<html lang="ko">
<head>
  <title>Healthcare LifeLink - 환자 상태</title>
        ...
        <main class="main_wrap sub_wrap">
                ...
                <div class="user_profile">
                  <div class="user_profile__img">
                      <c:if test="${userProfile ne null and userProfile ne 'null'}">
                          <img src="<%= contextPath %>/images/userProfile/${userProfile}" alt="" style="width:10vw;height:19vh; border-radius: 50%;">
                      </c:if>
                      <c:if test="${userProfile eq null or userProfile eq 'null' }">
                          <img src="<%= contextPath %>/modules/images/common/img_user.png" alt="">
                      </c:if>
                  </div>
                ...
                <ul class="user_state">
                  <li class="user_state__item">
                    <div class="user_objective__title _objective01">
                      ...
                      현재 <strong id ="step_realtime">0보</strong>! 산책 어때요?</p>
                    </div>
                  </li>
                  <li class="user_state__item">
                    <div class="user_objective__title _objective02">
                      <p class="user_objective__txt">어제 취침시간은  <strong id ="sleep_realtime" class="_red">13시간</strong>이에요.<br>
                      <strong id="sleep_target" > 개운한 하루 보내세요~</strong> </p>
                    </div>
                  </li>
                  <li class="user_state__item">
                    <div class="user_objective__title _objective03">
                      <p class="user_objective__txt">현재 스트레스는 <strong id='stress' class="_red">45</strong>이에요. <br> <strong id ="stress_target" >좋은하루 되세요.</strong></p>
                    </div>
                  </li>
                </ul>
              </div>
          ...
          <div class="flex_row">
            <section class="flex_item radius_box">
              ...
              <div class="radius_body">
                <p class="chat_txt"><img src="<%= contextPath %>/modules/images/common/img_chat_gpt.png" alt="">가 알려주는 나의 건강상태 <strong class="chat_txt_str">AI 주치의</strong>와 함께하세요.</p>
            ...
            <section class="flex_item radius_box">
              ...
              <div class="radius_body">
                <div class="img_box">
                  ...
                  <img src="<%= contextPath %>/modules/images/common/img_sample001.jpg" alt="">
                  </a>
                  <p style="margin-top: 10px;">국민건강보험공단 사칭 문자(스미싱) 주의하세요!</p>
                  <a href="https://www.nhis.or.kr/nhis/together/wbhaea01600m01.do?mode=view&articleNo=10841393" target="_blank" style="color: #3498db; text-decoration: none;">자세히보기 ></a>
                </div>
              </div>
            </section>
          </div>
...
<script src="<%= contextPath %>/js/biodata/realtimeBiodata.js" type="text/javascript"></script>
<script src="<%= contextPath %>/js/AI_chatgpt/getAIResult.js" type="text/javascript"></script>
<script>
    var searchUserId = '<%= userinfoId != null ? userinfoId : userId %>';
    ...
    $(document).ready(function(){
        ...
        <c:if test="${!empty acToken}">
           ...
           setInterval(function() { 
               realtimeBiodata(searchUserId); 
           }, 60000);
        </c:if>
        <c:if test="${empty acToken}">
            location.href = '<%= contextPath %>/user/signin';
        </c:if>
    });
...
function getHealthdata(searchUserId){
    ...
    $.ajax({
        type:"POST",
        url:"<%= contextPath %>/health/healthdailyData",
        dataType:"JSON",
