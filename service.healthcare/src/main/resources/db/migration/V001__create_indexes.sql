CREATE INDEX IF NOT EXISTS idx_health_data_user_date ON healthcare.health_data(user_seq, receive_time);
CREATE INDEX IF NOT EXISTS idx_health_data_user_year_month ON healthcare.health_data(user_seq, "YEAR", "MONTH");
CREATE INDEX IF NOT EXISTS idx_health_data_receive_time ON healthcare.health_data(receive_time);
CREATE INDEX IF NOT EXISTS idx_health_data_minute_user_time ON healthcare.health_data_minute(user_seq, receive_time);
CREATE INDEX IF NOT EXISTS idx_user_mng_user_id ON healthcare.user_mng(user_id);
CREATE INDEX IF NOT EXISTS idx_user_mng_user_seq ON healthcare.user_mng(user_seq);
CREATE INDEX IF NOT EXISTS idx_user_auth_info_user_seq ON healthcare.user_auth_info(user_seq);
CREATE INDEX IF NOT EXISTS idx_user_auth_info_role ON healthcare.user_auth_info(user_role_fk);
CREATE INDEX IF NOT EXISTS idx_doctor_mapping_user_seq ON healthcare.doctor_mapping_info(user_seq_fk);
CREATE INDEX IF NOT EXISTS idx_guardian_mapping_user_seq ON healthcare.guardian_mapping_info(user_seq_fk);
CREATE INDEX IF NOT EXISTS idx_health_score_user_date ON healthcare.health_score(user_seq, daily_date);
CREATE INDEX IF NOT EXISTS idx_community_commu_seq ON healthcare.community(commu_seq);
CREATE INDEX IF NOT EXISTS idx_community_user_seq ON healthcare.community(user_seq);
CREATE INDEX IF NOT EXISTS idx_community_reg_dt ON healthcare.community(reg_dt);

